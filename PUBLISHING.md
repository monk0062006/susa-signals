...
## npm — `@markerio-usa/core`, `@markerio-usa/browser`
...
npm pack --workspace @markerio-usa/core --dry-run   # inspect contents first
npm publish --workspace @markerio-usa/core
npm publish --workspace @markerio-usa/browser
```

`prepublishOnly` rebuilds, so a stale `dist/` cannot ship.

**Before the first publish:** bump both from `0.0.0`, and decide whether
`@markerio-usa/browser` ships the code-split chunks or a single file. It is
...
```swift
.package(url: "https://github.com/monk0062006/markerio-usa-core.git", from: "0.1.0")
```
