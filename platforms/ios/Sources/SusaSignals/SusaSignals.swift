...
public struct SusaSignalsConfig {
 ...
 */
public final class SusaSignals {
    private let config: SusaSignalsConfig
    private let consent: ConsentManager
    private let queue: SubmissionQueue
    private let client: IngestClient
    private let worker = DispatchQueue(label: "com.susatest.signals.feedback", qos: .utility)
    ...
    private static var instance: SusaSignals?
    ...
    @discardableResult
    public static func start(config: SusaSignalsConfig) -> SusaSignals {
        lock.lock()
        defer { lock.unlock() }

        if let instance { return instance }

        let sdk = SusaSignals(config: config)
        instance = sdk
        sdk.onInit()
        return sdk
    }

    public static func shared() -> SusaSignals? {
        lock.lock()
        defer { lock.unlock() }
        return instance
    }

    private init(config: SusaSignalsConfig) {
        self.config = config
    ...
    public func capture(
        ...
        overlayVisible = true
        let overlay = AnnotationComposerView(
            screenshot: image,
    ...
    private func log(_ message: String) {
        if !config.silent { print("[susa] \(message)") }
    }
}
